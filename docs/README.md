...
* **田中商事株式会社**（2014年4月 - 2015年9月）
  ...
  * 事業内容：電気材料及び電気器具の卸売、電設機材、照明、OA・住設機器などの総合卸販売
  ...
  [^1][テラハウスICAキャリア開発研究所](https://www.terahouse-ica.ac.jp/)
...
* **課題**
  * 以下のような課題点があり、このプロジェクトに至りました
    ...
    * 既存作成されていた資料では、要求範囲や機能一覧などが資料として作成されておらず、画面設計書しかなかった
    ...
    <br>資料がないまま、画面設計書に機能を落とし込んでいたため、システム化した際の業務フローに既存業務フローの漏れなどがあり、早急に仕様を変更する必要がありました。
* **実績・成果**
  * 既存作成されていた資料の見直し
  * 要件定義
  * 基本設計（UI/UX設計）
...
* **工夫した点**
  * 既存業務フローの理解を深めるために、社内ユーザーとのヒアリング、レビューを2ヶ月間ほぼ毎日実施しました。
  <br>既存業務を吸い上げ、新業務フローでは無駄のない業務フローの実現をするよう、仕様検討しました
...
* **工夫した点**
  * 従来のシステムでは、一部のボタンを押下するたびに、検索機能が走り、ページの再描画がされていました。<br>ページの再描画が走らないよう、jQuery,Knockout.jsのロジックをリファクタリングし、ユーザーストレスを軽減させることに努めました
...
* **課題**
  * 以下のような課題点があり、このプロジェクトに至りました
    * 工場では、手作業で行う工程が多く、人手による選別では良品・不良品の比率など品質情報の統計を取ることが難しくなっていた。
    <br>以上の点から品質改善につなげることができないでいました。
...
* **工夫した点**
  * 使用者目線に立ち、システムを検討することに苦労しました。
  ...
  <br>現場作業者へヒアリングを行い、システム化する際どのような手順なら、覚えやすいのかなど、操作のばらつきをなくすような設計を検討しました。
