...
大学を卒業後、2014年4月に田中商事株式会社へ入社しました。
2015年9月に退職後、職業訓練校でWeb開発の基礎を学習しました。
2016年4月に株式会社ネット-eへ入社しました。
2020年9月まで約4年半、工場の生産管理システム開発に従事し、要件定義や設計などの上流工程から、開発やテストまでを一貫して担当しました。
...
* **実績・成果**
  ...
  * 基本設計（UI/UX設計）
  ...
  業務知識が無いなか、手探り状態でしたが、既存システムの扱いについて経験豊富な社内ユーザーとレビューを重ね、既存業務フローを深く理解することができました。改善点、問題点を掘り下げることができ、なんとか設計書の完成をすることが出来ました。
...
* **課題**
  * 以下のような課題点があり、このプロジェクトに至りました。
    * 3年以上継続したシステムであり、導入パッケージのバージョンアップがされていおらず、今後の運用、機能追加に対応できるように、パッケージのアップデートを実行しました。
* **実績・成果**
  * Bootstrap v3.3.7→Bootstrap v4.6へのアップデート
  * jQuery v3.2.1→jQuery v3.5.1へのアップデート
  * Knockout.js v3.4.2→Knockout.js v3.5.1への アップデート
  * アップデートに伴い既存UIの変更・修正
  * WBS作成、更新（進捗管理）
...
* **工夫した点**
  ...
  * Bootstrapのレスポンシブ対応をうまく利用していなかったため、導入を実施し、これによりさらなる表示改善を実現する事ができました。
...
* **担当業務**
  ...
  * 結合テスト実施（内容検討、テスト仕様書作成、環境構築）
* **課題**
  * 工場では、日々手作業で行う工程が多く、作業者によって作業のバラツキがあり、人手による選別では、良品・不良品の比率など品質情報の統計を取ることもできず、品質改善につなげることが難しく、システム化されていない工程では、こういった品質問題が多発していました。
* **実績・成果**
  * 運用中のシステム保守、運用
  * システム未稼働の生産ラインへシステム導入
  * 手作業になっている工程に新システム導入（製品ラベル 作成～貼り付け作業）

  工程を見える化したことで、作業者の作業状況が把握ができるようになり、間違いを減らすことを実現しました。（システム導入後間違い0件）

* **工夫した点**
  * 使用者目線に立ち、システムを検討することに苦労しました。
  作業者にはITに疎い方も多く、操作できずに現工程に影響が出てしまうのではないかと懸念しました。現場作業者へヒアリングを行い、システム化する際どのような手順なら、覚えやすいかなど、操作のばらつきをなくすような設計を検討しました。
...
* **課題**
  * 初めてのプロジェクトであり、わからないことが多かったです。
  主要取引先の案件であり、長期に渡って運用されていたため、機能のデグレードテストを実施するため、既存仕様を理解する必要がありました。
* **実績・成果**
  * テスト項目の作成
  * テスト消化

  要因分析表、テスト観点表、デシジョンテーブルを用いて、テスト項目を作成しました。条件の組み合わせを明確にし、合計500件のテスト項目を作成しました。
  テスト項目を無事消化し、機能追加による決済端末システムのエンハンスによるデグレードがなく、既存の機能にも、影響のないことを確認できました。

* **工夫した点**
  * 仕様を理解するために、仕様書を読み解くだけでなく、開発者にレビューを実施してもらうよう提案
  * テスト項目に漏れがないテスト仕様書を作成するために、２ヶ月毎日レビューの実施
